	...
	*/
	...
	if (initFileFilter)
	{
		MadEdit::m_FileFilter = wxString(wxGetTranslation(_T("All files (*;*.*)"))) + wxT("|") + wxFileSelectorDefaultWildcardStr + wxT("|") + wxGetTranslation(_T("Plain Text (*.txt)")) + 
			...
			wxT("*.conf;*.htaccess|Bash Shell Script (*.sh;*.bsh;*.configure)|*.sh;*.bsh;*.configure|Boo (*.boo)|*.boo|C (*.c;*.h)|*.c;*.h|")
			...
			wxT("LaTeX (*.aux;*.sty;*.tex)|*.aux;*.sty;*.tex|Lisp (*.cl;*.lisp)|*.cl;*.lisp|Lout (*.lt)|*.lt|Lua (*.lua)|*.lua|MadPython (*.mpy)|*.mpy|MadEdit Syntax (*.syn)|*.syn|Mako (*.mako;*.mao)|")
			...
			wxT("*.ado;*.do|System Verilog (*.sv;*.svh)|*.sv;*.svh|Tcl/Tk (*.itcl;*.tcl;*.tk)|*.itcl;*.tcl;*.tk|TeX/LaTeX (*.tex)|*.tex|Vala (*.vala)|*.vala|")
			wxT("VBScript (*.dsm;*.vbs)|*.dsm;*.vbs|Verilog (*.v)|*.v|VHDL (*.vh;*.vhd;*.vhdl)|*.vh;*.vhd;*.vhdl|Visual Basic (*.bas;*.cls;*.frm;*.vb)|")
			wxT("*.bas;*.cls;*.frm;*.vb|Windows Registry File (*.reg)|*.reg|XML (*.axl;*.dtd;*.plist;*.rdf;*.svg;*.xml;*.xrc;*.xsd;*.xsl;*.xslt;*.xul)|*.axl;*.dtd;*.plist;*.rdf;*.svg;*.xml;")
...
{
	event.Enable(g_ActiveMadEdit && !g_ActiveMadEdit->IsHexMode() && g_ActiveMadEdit->HasBookMark());
}
void MadEditFrame::OnUpdateUI_MenuEditCheckBookmarkWritable(wxUpdateUIEvent& event)
{
	event.Enable(g_ActiveMadEdit && !g_ActiveMadEdit->IsReadOnly() && !g_ActiveMadEdit->IsHexMode() && g_ActiveMadEdit->HasBookMark());
}
//----------
...
{
	event.Enable(g_ActiveMadEdit && g_ActiveMadEdit->IsColumnMode() && g_ActiveMadEdit->IsSelected());
}
...
{
	event.Enable(g_ActiveMadEdit && !g_ActiveMadEdit->IsHexMode() && g_ActiveMadEdit->HasBracePair());
}
...
{
	event.Enable(g_ActiveMadEdit && !IsMacroRecording());
...
{
	event.Enable(g_ActiveMadEdit && g_ActiveMadEdit->IsModified());
}
...
{
	event.Enable(g_ActiveMadEdit && (!g_ActiveMadEdit->IsHexMode()) && IsMacroStopped());
}

void MadEditFrame::OnUpdateUI_MenuToolsStopRecMacro(wxUpdateUIEvent& event)
{
	event.Enable(g_ActiveMadEdit && (!g_ActiveMadEdit->IsHexMode()) && IsMacroRecording());
}

void MadEditFrame::OnUpdateUI_MenuToolsPlayRecMacro(wxUpdateUIEvent& event)
{
	event.Enable(g_ActiveMadEdit && (!g_ActiveMadEdit->IsHexMode()) && IsMacroStopped() && HasRecordedScript());
}
...
void MadEditFrame::OnEditSortAscending(wxCommandEvent& WXUNUSED(event))
{
	if (g_ActiveMadEdit && !g_ActiveMadEdit->IsHexMode())
	{
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString::Format(wxT("SortLines(MadSortLinesFlags.Ascending, %d, %d) # (MadSortLinesFlags, beginline, endline)"), begin, end));
	}
}
...
void MadEditFrame::OnEditSortDescending(wxCommandEvent& WXUNUSED(event))
{
	if (g_ActiveMadEdit && !g_ActiveMadEdit->IsHexMode())
	{
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString::Format(wxT("SortLines(MadSortLinesFlags.Descending, %d, %d) # (MadSortLinesFlags, beginline, endline)"), begin, end));
	}
}
...
void MadEditFrame::OnEditSortAscendingCase(wxCommandEvent& WXUNUSED(event))
{
	if (g_ActiveMadEdit && !g_ActiveMadEdit->IsHexMode())
	{
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString::Format(wxT("SortLines(MadSortLinesFlags.Ascending|MadSortLinesFlags.CaseSensitive, %d, %d) # (MadSortLinesFlags, beginline, endline)"), begin, end));
	}
}
...
void MadEditFrame::OnEditSortDescendingCase(wxCommandEvent& WXUNUSED(event))
{
	if (g_ActiveMadEdit && !g_ActiveMadEdit->IsHexMode())
	{
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString::Format(wxT("SortLines(MadSortLinesFlags.Descending|MadSortLinesFlags.CaseSensitive, %d, %d) # (MadSortLinesFlags, beginline, endline)"), begin, end));
	}
}
...
void MadEditFrame::OnEditSortByOptions(wxCommandEvent& WXUNUSED(event))
{
	if (g_ActiveMadEdit && !g_ActiveMadEdit->IsHexMode())
	{
		...
		if (IsMacroRecording())
		{
			...
			RecordAsMadMacro(g_ActiveMadEdit, wxString::Format(wxT("SortLines(%s, %d, %d) # (MadSortLinesFlags, beginline, endline)"), sflags.c_str(), begin, end));
		}
	}
}
...
void MadEditFrame::OnEditIncIndent(wxCommandEvent& WXUNUSED(event))
{
	if (g_ActiveMadEdit)
	{
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("IncreaseIndent()")));
	}
}
void MadEditFrame::OnEditDecIndent(wxCommandEvent& WXUNUSED(event))
{
	if (g_ActiveMadEdit)
	{
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("DecreaseIndent()")));
	}
}
...
void MadEditFrame::OnEditComment(wxCommandEvent& WXUNUSED(event))
{
	if (g_ActiveMadEdit)
	{
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("Comment()")));
	}
}
...
void MadEditFrame::OnEditUncomment(wxCommandEvent& WXUNUSED(event))
{
	if (g_ActiveMadEdit)
	{
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("Uncomment()")));
	}
}
...
void MadEditFrame::OnEditToHalfWidthByOptions(wxCommandEvent& WXUNUSED(event))
{
	...
	if (sels > 0)
	{
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString::Format(wxT("ToHalfWidth(%s, %s, %s, %s) # (ascii, japanese, korean, other)"), ascii ? wxT("True") : wxT("False"),
...
void MadEditFrame::OnEditToFullWidthByOptions(wxCommandEvent& WXUNUSED(event))
{
	...
	if (sels > 0)
	{
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString::Format(wxT("ToFullWidth(%s, %s, %s, %s) # (ascii, japanese, korean, other)"), ascii ? wxT("True") : wxT("False"),
							  japanese ? wxT("True") : wxT("False"), korean ? wxT("True") : wxT("False"), other ? wxT("True") : wxT("False")));
	}
}
...
void MadEditFrame::OnEditInsertNumbers(wxCommandEvent& WXUNUSED(event))
{
	if (g_ActiveMadEdit && g_ActiveMadEdit->IsColumnMode())
	{
		...
		if (g_MadNumberDlg->ShowModal() == wxID_OK)
		{
			...
			if (IsMacroRecording())
				RecordAsMadMacro(g_ActiveMadEdit, wxString::Format(wxT("InsertIncrementalNumber(%s, %s, %s, %s, %s, %s, %s, \"%s\", \"%s\") # (initial, step, total, stepType, fmt, align, zeroPad, pref, post)"),
...
void MadEditFrame::OnSearchGoToLine(wxCommandEvent& WXUNUSED(event))
{
	...
	if (!str.IsEmpty() && str.ToLong(&line, base))
	{
		...
		if (IsMacroRecording())
		{
			if (base == 16)
			{
				RecordAsMadMacro(g_ActiveMadEdit, wxString::Format(wxT("GoToLine(0x%X) # line"), line));
			}
			else
			{
...
void MadEditFrame::OnSearchGoToPosition(wxCommandEvent& WXUNUSED(event))
{
	...
	if (!str.IsEmpty())
	{
		...
		if (StrToInt64(str, pos))
		{
			...
			if (IsMacroRecording())
				RecordAsMadMacro(g_ActiveMadEdit, wxString::Format(wxT("SetCaretPosition(%s) # position"), (wxPos1.ToString()).c_str()));
		}
	}
}
	...
	{ 
		...
		if (!str.IsEmpty())
		{
			...
			if (tkz.HasMoreTokens())
			{
				...
				if (IsMacroRecording())
					RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("SetEncoding(\"")) + enc + wxT("\") # encname"));
...
void MadEditFrame::OnViewSyntax(wxCommandEvent& event)
{
	...
	{ 
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("SetSyntax(\"")) + title + wxT("\") # title"));
	}
}
...
void MadEditFrame::OnViewFontName(wxCommandEvent& event)
{
	...
	{ 
        ...
        if (IsMacroRecording())
            RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("SetFontA(")) + fontname + wxT(", ") + wxString::Format("%d", fs) + wxT(") # Font name, size"));
	}
}
...
void MadEditFrame::OnViewFixedWidthMode(wxCommandEvent& event)
{
	...
	{ 
		...
		if (IsMacroRecording())
		{
			wxString recstr(event.IsChecked() ? wxT("SetFixedWidthMode(mode=True)") : wxT("SetFixedWidthMode(mode=False)"));
			RecordAsMadMacro(g_ActiveMadEdit, recstr);
		}
	}
}
...
void MadEditFrame::OnViewLineSpacing(wxCommandEvent& event)
{
	...
	{ 
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString::Format(wxT("SetLineSpacing(%d) # percent number"), ls));
	}
}
...
void MadEditFrame::OnViewNoWrap(wxCommandEvent& WXUNUSED(event))
{
	...
	{ 
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("NoWrap()")));
	}
}
...
void MadEditFrame::OnViewWrapByWindow(wxCommandEvent& WXUNUSED(event))
{
	...
	{ 
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("WrapByWindow()")));
	}
}
void MadEditFrame::OnViewWrapByColumn(wxCommandEvent& WXUNUSED(event))
{
	...
	{ 
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("WrapByColumn()")));
	}
}
...
void MadEditFrame::OnViewDisplayLineNumber(wxCommandEvent& event)
{
	...
	{ 
		...
		if (IsMacroRecording())
		{
			if (event.IsChecked())
			{
				RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("DisplayLineNumber()")));
			}
			else
			{
				RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("HideLineNumber()")));
			}
		}
	}
}
...
void MadEditFrame::OnViewDisplayBookmark(wxCommandEvent& event)
{
	...
	{ 
		...
		if (IsMacroRecording())
		{
			if (event.IsChecked())
			{
					RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("DisplayBookmark()")));
			}
			else
			{
					RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("HideBookmark()")));
			}
		}
	}
}
...
void MadEditFrame::OnViewShowEndOfLine(wxCommandEvent& event)
{
	...
	{ 
		...
		if (IsMacroRecording())
		{
			if (event.IsChecked())
			{
				RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("ShowEndOfLine()")));
			}
			else
			{
				RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("HideEndOfLine()")));
			}
		}
	}
}
...
void MadEditFrame::OnViewShowTabChar(wxCommandEvent& event)
{
	...
	{ 
		...
		if (IsMacroRecording())
		{
			if (event.IsChecked())
			{
				RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("ShowTabChar()")));
			}
			else
			{
				RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("TabChar()")));
			}
		}
	}
}
...
void MadEditFrame::OnViewShowSpaceChar(wxCommandEvent& event)
{
	...
	{ 
		...
		if (IsMacroRecording())
		{
			if (event.IsChecked())
			{
				RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("ShowSpaceChar()")));
			}
			else
			{
				RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("HideSpaceChar()")));
			}
		}
	}
}
...
void MadEditFrame::OnViewShowAllChars(wxCommandEvent& event)
{
	...
	{ 
		...
		if (IsMacroRecording())
		{
			...
			{
				RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("HideTabChar()")));
				RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("HideSpaceChar()")));
				RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("HideEndOfLine()")));
			}
		}
...
void MadEditFrame::OnViewMarkActiveLine(wxCommandEvent& event)
{
	if (g_ActiveMadEdit)
	{
		...
		if (IsMacroRecording())
		{
			if (event.IsChecked())
			{
				RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("MarkActiveLine()")));
			}
			else
			{
				RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("HideMarkActiveLine()")));
			}
		}
	}
}
...
void MadEditFrame::OnViewTextMode(wxCommandEvent& WXUNUSED(event))
{
	if (g_ActiveMadEdit)
	{
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("TextMode()")));
	}
}
...
void MadEditFrame::OnViewColumnMode(wxCommandEvent& WXUNUSED(event))
{
	if (g_ActiveMadEdit)
	{
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("ColumnMode()")));
	}
}
...
void MadEditFrame::OnViewHexMode(wxCommandEvent& WXUNUSED(event))
{
	if (g_ActiveMadEdit)
	{
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("HexMode()")));
	}
}
...
void MadEditFrame::OnToolsPlayRecMacro(wxCommandEvent& WXUNUSED(event))
{
	if (g_ActiveMadEdit)
	{
		...
		wxString medit(wxT("me.")), pyscript;
		...
		if (total > 0)
		{
			...
			pyscript = wxString(wxT("#Create MadEdit Object for active edit")) + endline + wxT("me = MadEdit()") + endline + endline;
	...
	if (!filename.IsEmpty())
	{
		...
		if (scriptfile.IsOpened())
		{
			...
			if (total)
			{
				...
				scriptfile.AddLine(wxT("me = MadEdit()"));
...
void MadEditFrame::OnToolsSimp2TradChinese(wxCommandEvent& WXUNUSED(event))
{
	...
	{ 
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("ConvertChineseA(MadConvertEncodingFlag.SC2TC) # Flag")));
	}
}
...
void MadEditFrame::OnToolsTrad2SimpChinese(wxCommandEvent& WXUNUSED(event))
{
	...
	{ 
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("ConvertChineseA(MadConvertEncodingFlag.TC2SC) # Flag")));
	}
}
...
void MadEditFrame::OnToolsKanji2TradChinese(wxCommandEvent& WXUNUSED(event))
{
	...
	{ 
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("ConvertChineseA(MadConvertEncodingFlag.JK2TC) # Flag")));
	}
}
...
void MadEditFrame::OnToolsKanji2SimpChinese(wxCommandEvent& WXUNUSED(event))
{
	...
	{ 
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("ConvertChineseA(MadConvertEncodingFlag.JK2SC) # Flag")));
	}
}
...
void MadEditFrame::OnToolsChinese2Kanji(wxCommandEvent& WXUNUSED(event))
{
	...
	{ 
		...
		if (IsMacroRecording())
			RecordAsMadMacro(g_ActiveMadEdit, wxString(wxT("ConvertChineseA(MadConvertEncodingFlag.C2JK) # Flag")));
	}
}
...
void MadEditFrame::OnToolsSimp2TradClipboard(wxCommandEvent& WXUNUSED(event))
{
	...
	if (IsMacroRecording())
		...
		m_MadMacroScripts.Add(wxString(wxT("ConvertChineseInClipboard(MadConvertChineseFlag.SC2TC) # Flag")));
}
...
void MadEditFrame::OnToolsTrad2SimpClipboard(wxCommandEvent& WXUNUSED(event))
{
	...
	if (IsMacroRecording())
		...
		m_MadMacroScripts.Add(wxString(wxT("ConvertChineseInClipboard(MadConvertChineseFlag.Trad2Simp) # Flag")));
}
...
void MadEditFrame::OnToolsKanji2TradClipboard(wxCommandEvent& WXUNUSED(event))
{
	...
	m_MadMacroScripts.Add(wxString(wxT("ConvertChineseInClipboard(MadConvertChineseFlag.Kanji2Trad) # Flag")));
}
...
void MadEditFrame::OnToolsKanji2SimpClipboard(wxCommandEvent& WXUNUSED(event))
{
	...
	if (IsMacroRecording())
		...
		m_MadMacroScripts.Add(wxString(wxT("ConvertChineseInClipboard(MadConvertChineseFlag.Kanji2Simp) # Flag")));
}
...
void MadEditFrame::OnToolsChinese2KanjiClipboard(wxCommandEvent& WXUNUSED(event))
{
	...
	if (IsMacroRecording())
		...
		m_MadMacroScripts.Add(wxString(wxT("ConvertChineseInClipboard(MadConvertChineseFlag.Chinese2Kanji) # Flag")));
}
