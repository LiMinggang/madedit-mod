...
MadUndo::~MadUndo()
{
	if(m_Undos.size())
	{
		...
		do
		{
			...
			case udtInsert:
				{
					MadInsertUndoData *pd = static_cast <MadInsertUndoData*>(pud);
					wxASSERT(pd);
					//pd->m_Data.clear();
					delete pd;
				}
			...
			case udtDelete:
				{
					MadDeleteUndoData *pd = static_cast <MadDeleteUndoData*>(pud);
					wxASSERT(pd);
					//pd->m_Data.clear();
					delete pd;
				}
			...
			case udtOverwrite:
				{
					MadOverwriteUndoData *pd = static_cast <MadOverwriteUndoData*>(pud);
					wxASSERT(pd);
					//pd->m_DelData.clear();
					//pd->m_InsData.clear();
					delete pd;
